    ...
    <div class="col-xxl-12">
        <div class="card my-4">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h5 class="mb-0">{{ $payment_method->name }}</h5>
                <div>
                    @can('update', $payment_method)
                    <a href="{{ route('payment_methods.edit', $payment_method) }}" class="btn btn-warning btn-sm me-2">
                        <i class="icon-base bx bx-edit me-1"></i> Éditer
                    </a>
                    @endcan
                    @can('delete', $payment_method)
                    <form action="{{ route('payment_methods.destroy', $payment_method) }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
                ...
                <table class="table table-borderless">
                    <tbody class="table-border-bottom-0">
                        <tr>
                            ...
                            <td><span class="badge bg-light text-dark">{{ $payment_method->code }}</span></td>
                        </tr>
                    </tbody>
                </table>
            </div>
